...
class QuestionnaireExams:
    def __init__(self, questionnaire_string, number_of_questions=None):
        self._questions, self._answers = (
            questionnaire_string.split("#")[::2],
            questionnaire_string.split("#")[1::2],
        )
        self.questionnaire = [
        ...
        self.number_of_questions = (
            len(self._questions) if number_of_questions is None else number_of_questions
        )
...
def do_exam_setup():
    print(
        "Write directory file with questions and answers (answers must be betweenn #):"
    )
    exam_file = input()
